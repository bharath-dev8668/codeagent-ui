...
export default function Page() {
  ...
  const sendWithContent = async (txt: string, files: File[]) => {
    ...
    try {
      ...
      setThinking(false);
      const tools: string[] = [];
      if (reply.includes('```')) tools.push('📝 Code written');
      if (/bug|fix|error/i.test(reply)) tools.push('🐛 Bug fixed');
      if (/refactor|improve/i.test(reply)) tools.push('♻️ Refactored');
      if (/best practice|recommend/i.test(reply)) tools.push('💡 Best practices');
      setMsgs(m => [...m, { role: 'agent', html: fmt(reply), tools }]);
      setStatus({ label: 'Ready', cls: 'sp-ready' });
    } catch (err: any) {
      setThinking(false);
      setMsgs(m => [...m, { role: 'agent', html: fmt(`❌ **Error:** ${err.message}\n\nTHIS AGENT CANNOT READ IMAGES TRY ANOTHER MODEL.`) }]);
  ...
  const clearChat = () => { if (!confirm('Clear chat?')) return; setMsgs([]); histRef.current = []; setChatFiles([]); };
      ...
      <aside className={`sidebar ${sbOpen ? 'open' : ''}`} onMouseEnter={() => clearTimeout(sbCloseTimer.current)} onMouseLeave={closeSidebar}>
        ...
        <div className="sb-body">
          ...
          <details className="inst-details">
            ...
            <div className="inst-box">You&apos;re my ruthless mentor. Don&apos;t sugarcoat anything. If my idea is weak, call it trash and tell me why. Your job is to stress-test everything I say until it&apos;s bulletproof. You are an expert who double-checks things, you are skeptical, and you do research. I am not always right. Neither are you, but we both strive for accuracy. Be playful and goofy.</div>
          </details>
        </div>
        <div className="sb-foot">Powered by <strong>Groq AI</strong> · Free &amp; Unlimited<br />Built by <strong>Bharath Thommandru</strong></div>
      </aside>
        ...
        {screen === 'home' && (
          <div className="home" id="homeScreen">
            ...
            <div className="home-text">
              ...
              <p className="home-sub">Build something amazing — just start typing below.</p>
            </div>
            <div className="home-input-wrap">
              ...
              <div className="glass-input">
                <textarea ref={homeInpRef} id="homeInp" placeholder="Type your request..." rows={1}
            ...
            <div className="ciarea">
              ...
              <div className="cglass">
                <div className="cglass-top">
                  <textarea ref={chatInpRef} id="chatInp" placeholder="Ask me to write, fix, explain or refactor…" rows={1}
              ...
              <div className="cfoot">
                ...
                <span className="cfoot-r">Built by <strong>Bharath Thommandru</strong></span>
              </div>
            </div>
          </div>
        )}
